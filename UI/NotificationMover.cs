...
namespace Gw2EventTracker.UI {
    ...
    internal sealed class NotificationMover : Control {
        ...
        public NotificationMover(params ScreenRegion[] screenPositions) : this((IEnumerable<ScreenRegion>)screenPositions) {
        }
        ...
        public NotificationMover(IEnumerable<ScreenRegion> screenPositions) {
            ...
            _previewTexture = EventTrackerModule.Instance.ContentsManager.GetTexture("textures/ns-button.png");
        }
        ...
        protected override void Paint(SpriteBatch spriteBatch, Rectangle bounds) {
            ...
            foreach (var region in _screenRegions) {
                ...
                DrawNotificationPreviews(spriteBatch, region);
            ...
            spriteBatch.DrawStringOnCtrl(this, "Drag to set where notifications appear. Press ESC to close.", GameService.Content.DefaultFont32, bounds, Color.White, false, HorizontalAlignment.Center);
        }

        private void DrawNotificationPreviews(SpriteBatch spriteBatch, ScreenRegion region) {
            for (var i = 0; i < PreviewCount; i++) {
                var top = region.Location.Y + (EventNotification.NotificationHeight + EventNotification.NotificationStackSpacing) * i;
                var previewBounds = new Rectangle(
                    region.Location.X,
                    top,
                    EventNotification.NotificationWidth,
                    EventNotification.NotificationHeight);

                spriteBatch.DrawOnCtrl(this, _previewTexture, previewBounds, Color.White * 0.85f);
                spriteBatch.DrawStringOnCtrl(
                    this,
                    i == 0 ? "Sample Event Alert" : string.Empty,
                    GameService.Content.DefaultFont14,
                    new Rectangle(previewBounds.X + EventNotification.NotificationHeight + 10, previewBounds.Y + 8, previewBounds.Width - EventNotification.NotificationHeight - 20, previewBounds.Height / 2),
                    Color.White,
                    false,
